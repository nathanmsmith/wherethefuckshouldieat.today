...
# constants
kRestaurantName = "Restaurant Name"
kRestaurantHours = "Restaurant Hours"
kRestaurantKitchens = "Restaurant Kitchens"
kKitchenName = "Kitchen Name"
kKitchenItems = "Kitchen Items"
kEntreeName = "Entree Name"
kType = "Type"
...
class MenuParser:
    ...
    def setDiningHours(self, restaurantName):
        if restaurantName == "Covel Dining":
            return range(12+5, 12+9)
        elif restaurantName == "Bruin Plate" or restaurantName == "De Neve Dining":
            return range(12+5, 12+8)
        else:
            return range(0)
    ...
    def parseRestaurantHTML(self, html, shouldGetNutrition):
        ...
        for index, values in enumerate(data):
            restaurant = {kRestaurantName: "",
                          kRestaurantKitchens: [], kRestaurantHours: range(0)}
            for d in values:
                        ...
                        for child in d.children:
                            if child.name == 'a':
                                ...
                                if (childclass and childclass[0] == 'menuloclink'):
                                    ...
                                    restaurant[kRestaurantHours] = self.setDiningHours(restaurant[kRestaurantName])
                                ...
                                for li in ul:
                                    if li.name == 'li':
                                        ...
                                        if liclass and 'category' in liclass[0]:
                                            ...
                                            # determine whether is 'vegetarian'
                                            # or 'vegan'
                                            itemType = "o"
                                            img = li.find('img')
                                            if img:
                                            ...
                                            if shouldGetNutrition:
                                                # get the link to nutrition
                                                # data & the nutrition data
                                                nutrition = {}
                                                nutritionURL = li.find(
                                                    'a').get('href')
                                                nutritionURL = re.findall(
                                                    r'RecipeNumber=\d+', nutritionURL)
                                                ...
                                                if nutritionURL != None and len(nutritionURL) == 6:
                                                    ...
                                                    if os.path.exists(nutritionPath) and os.path.isfile(nutritionPath):
                                                        ...
                                                        nutrition = json.loads(
                                                            nutritionJSON)
                                                entree = {
                                                    kEntreeName: line, kNutritionData: nutrition, kType: itemType}
                                            else:
                                                entree = line
                                            # print(entree)
                                            kitchen[kKitchenItems].append(
                                                entree)
                        # print(kitchen)
...
def downloadMenu(date, dateString, meal, shouldGetNutrition):
    ...
    if shouldGetNutrition:
        if meal == Meal.breakfast:
            print("Downloading menus with nutrition for: " +
                  dateString + " [BREAKFAST]")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "year", help="year of the menu data you wish to download", type=int)
    parser.add_argument(
        "month", help="month of the menu data you wish to download", type=int)
    parser.add_argument(
        "day", help="month of the menu data you wish to download", type=int)
    parser.add_argument(
        "-n", "--nutrition", help="determine whether to download nutrition data", action="store_true")
    parser.add_argument("-b", "--breakfast",
                        help="download breakfast menus", action="store_true")
    parser.add_argument(
        "-l", "--lunch", help="download lunch menus", action="store_true")
    parser.add_argument(
        "-d", "--dinner", help="download dinner menus", action="store_true")
    ...
    if (not args.breakfast) and (not args.lunch) and (not args.dinner):
        menus["b"] = downloadMenu(
            date, dateString, Meal.breakfast, shouldGetNutrition)
        menus["l"] = downloadMenu(
            date, dateString, Meal.lunch, shouldGetNutrition)
        menus["d"] = downloadMenu(
            date, dateString, Meal.dinner, shouldGetNutrition)
    else:
        if args.breakfast:
            menus["b"] = downloadMenu(
                date, dateString, Meal.breakfast, shouldGetNutrition)
